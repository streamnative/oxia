...
type ReplicateStreamProvider interface {
	GetReplicateStream(ctx context.Context, follower string, namespace string, shard int64, term int64, commitContext *kv.CommitContext) (proto.OxiaLogReplication_ReplicateClient, error)
...
func (fc *followerCursor) streamEntries() error {
	...
	fc.Lock()
	var err error
	if fc.stream, err = fc.replicateStreamProvider.GetReplicateStream(ctx, fc.follower, fc.namespace, fc.shardId, fc.term, fc.db.ReadCommitContext()); err != nil {
		fc.Unlock()
		return err
	}
	fc.Unlock()
