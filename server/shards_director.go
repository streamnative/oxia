...
type ShardsDirector interface {
	...
	GetOrCreateFollower(namespace string, shardId int64, term int64, commitContext *kv.CommitContext) (FollowerController, error)
...
func (s *shardsDirector) GetOrCreateFollower(namespace string, shardId int64, term int64, commitContext *kv.CommitContext) (FollowerController, error) {
	s.Lock()
	defer s.Unlock()
	...
	fc, err := NewFollowerControllerWithContext(s.config, namespace, shardId, s.walFactory, s.kvFactory, commitContext)
	if err != nil {
		return nil, err
	}
