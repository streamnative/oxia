...
func newServer(t *testing.T) (s *server.Server, addr model.NodeInfo) {
	t.Helper()
...
func TestCoordinator_LeaderFailover(t *testing.T) {
	...
	s2, sa2 := newServer(t)
	s3, sa3 := newServer(t)
	servers := map[model.NodeInfo]*server.Server{
		sa1: s1,
		sa2: s2,
		sa3: s3,
	}
...
func TestCoordinator_RebalanceCluster(t *testing.T) {
	...
	assert.EqualValues(t, 2, len(ns1Status.Shards))
	assert.EqualValues(t, 3, ns1Status.ReplicationFactor)
	checkServerLists(t, []model.NodeInfo{sa1, sa2, sa3}, ns1Status.Shards[0].Ensemble)
	checkServerLists(t, []model.NodeInfo{sa1, sa2, sa3}, ns1Status.Shards[1].Ensemble)

	// Add `s4` and remove `s1` from the cluster config
	mutex.Lock()
	clusterConfig.Servers = []model.NodeInfo{sa2, sa3, sa4}
	mutex.Unlock()
	...
	assert.EqualValues(t, 2, len(ns1Status.Shards))
	assert.EqualValues(t, 3, ns1Status.ReplicationFactor)
	checkServerLists(t, []model.NodeInfo{sa2, sa3, sa4}, ns1Status.Shards[0].Ensemble)
	checkServerLists(t, []model.NodeInfo{sa2, sa3, sa4}, ns1Status.Shards[1].Ensemble)
...
func TestCoordinator_ShrinkCluster(t *testing.T) {
	...
	clusterConfig := model.ClusterConfig{
		...
		Servers: []model.NodeInfo{sa1, sa2, sa3, sa4},
	}
	clientPool := common.NewClientPool(nil, nil)
...
func checkServerLists(t *testing.T, expected, actual []model.NodeInfo) {
	t.Helper()
...
func TestCoordinator_RefreshServerInfo(t *testing.T) {
	...
	clusterServer := make([]model.NodeInfo, 0)
	for _, sv := range clusterConfig.Servers {
		clusterServer = append(clusterServer, model.NodeInfo{
