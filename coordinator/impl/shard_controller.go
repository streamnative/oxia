...
type ShardController interface {
	...
	HandleNodeFailure(failedNode model.NodeInfo)

	SyncServerAddress()

	SwapNode(from model.NodeInfo, to model.NodeInfo) error
	DeleteShard()
...
type shardController struct {
	...
	electionOp              chan any
	deleteOp                chan any
	nodeFailureOp           chan model.NodeInfo
...
func NewShardController(namespace string, shard int64, namespaceConfig *model.NamespaceConfig,
	...
	s := &shardController{
		...
		nodeFailureOp:           make(chan model.NodeInfo, chanBufferSize),
...
func (s *shardController) HandleNodeFailure(failedNode model.NodeInfo) {
...
func (s *shardController) electLeader() error {
	...
	if s.log.Enabled(context.Background(), slog.LevelInfo) {
		f := make([]struct {
			ServerAddress model.NodeInfo `json:"server-address"`
...
func (s *shardController) getRefreshedEnsemble() []model.NodeInfo {
	currentEnsemble := s.shardMetadata.Ensemble
	refreshedEnsembleServiceAddress := make([]model.NodeInfo, len(currentEnsemble))
...
func (s *shardController) keepFencingFailedFollowers(successfulFollowers map[model.NodeInfo]*proto.EntryId) {
...
func (s *shardController) keepFencingFollower(ctx context.Context, node model.NodeInfo) {
	s.log.Info(
...
func (s *shardController) newTermAndAddFollower(ctx context.Context, node model.NodeInfo) error {
	res := make(chan error)
...
func (s *shardController) internalNewTermAndAddFollower(ctx context.Context, node model.NodeInfo, res chan error) {
...
func (s *shardController) newTermQuorum() (map[model.NodeInfo]*proto.EntryId, error) { //nolint:revive
			...
			map[string]string{
				...
				if err != nil {
					s.log.Warn(
						...
						slog.Any("error", err),
						slog.String("node", serverAddress.GetID()),
					)
				} else {
					s.log.Info(
	...
	res := make(map[model.NodeInfo]*proto.EntryId)
	var err error
	...
	for successResponses < majority && totalResponses < fencingQuorumSize {
		...
		if r.error == nil {
			...
			if listContains(s.shardMetadata.Ensemble, r.NodeInfo) {
				res[r.NodeInfo] = r.EntryId
			}
		} else {
...
func (s *shardController) newTerm(ctx context.Context, node model.NodeInfo) (*proto.EntryId, error) {
...
func (s *shardController) deleteShardRpc(ctx context.Context, node model.NodeInfo) error {
...
func selectNewLeader(newTermResponses map[model.NodeInfo]*proto.EntryId) (
	leader model.NodeInfo, followers map[model.NodeInfo]*proto.EntryId) {
	...
	for addr, headEntryId := range newTermResponses {
		if headEntryId.Term > currentMaxTerm {
			...
			candidates = []model.NodeInfo{addr}
	...
	followers = make(map[model.NodeInfo]*proto.EntryId)
...
func (s *shardController) becomeLeader(leader model.NodeInfo, followers map[model.NodeInfo]*proto.EntryId) error {
	timer := s.leaderElectionLatency.Timer()
...
func (s *shardController) addFollower(leader model.NodeInfo, follower string, followerHeadEntryId *proto.EntryId) error {
...
func (s *shardController) Leader() *model.NodeInfo {
	s.shardMetadataMutex.Lock()
	defer s.shardMetadataMutex.Unlock()
...
func (s *shardController) SwapNode(from model.NodeInfo, to model.NodeInfo) error {
	res := make(chan error)
...
func (s *shardController) swapNode(from model.NodeInfo, to model.NodeInfo, res chan error) {
	s.shardMetadataMutex.Lock()
...
func (s *shardController) isFollowerCatchUp(ctx context.Context, server model.NodeInfo, leaderHeadOffset int64) error {
...
func (s *shardController) waitForFollowersToCatchUp(ctx context.Context, leader model.NodeInfo, ensemble []model.NodeInfo) error {
...
func listContains(list []model.NodeInfo, sa model.NodeInfo) bool {
...
func replaceInList(list []model.NodeInfo, oldServerAddress, newServerAddress model.NodeInfo) []model.NodeInfo {
	var res []model.NodeInfo
	for _, item := range list {
