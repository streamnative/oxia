...
type NodeAvailabilityListener interface {
	NodeBecameUnavailable(node model.NodeInfo)
}
...
type Coordinator interface {
	...
	FindServerAddressByInternalAddress(internalAddress string) (*model.NodeInfo, bool)
...
func (c *coordinator) NodeBecameUnavailable(node model.NodeInfo) {
	c.Lock()
...
func (c *coordinator) FindServerAddressByInternalAddress(internalAddress string) (*model.NodeInfo, bool) {
	if info, exist := c.serverIndexes.Load(internalAddress); exist {
		address, ok := info.(model.NodeInfo)
		if !ok {
...
func (*coordinator) findServerByInternalAddress(newClusterConfig model.ClusterConfig, server string) *model.NodeInfo {
