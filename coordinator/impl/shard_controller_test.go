...
func TestLeaderElection_ShouldChooseHighestTerm(t *testing.T) {
	tests := []struct {
		name                   string
		candidates             map[model.ServerInfo]*proto.EntryId
		expectedLeader         model.ServerInfo
		expectedFollowersCount int
		expectedFollowers      map[model.ServerInfo]*proto.EntryId
	}{
		{
			...
			expectedLeader:         model.ServerInfo{Public: "2", Internal: "2"},
		...
		},
		{
			...
			expectedLeader:         model.ServerInfo{Public: "1", Internal: "1"},
			expectedFollowersCount: 2,
			expectedFollowers: map[model.ServerInfo]*proto.EntryId{
				{Public: "2", Internal: "2"}: {Term: 198, Offset: 1500},
				{Public: "3", Internal: "3"}: {Term: 199, Offset: 1500},
...
func TestShardController(t *testing.T) {
	...
	s1 := model.ServerInfo{Public: "s1:9091", Internal: "s1:8191"}
	s2 := model.ServerInfo{Public: "s2:9091", Internal: "s2:8191"}
	s3 := model.ServerInfo{Public: "s3:9091", Internal: "s3:8191"}
...
func (m *mockCoordinator) FindServerAddressByInternalAddress(_ string) (*model.ServerInfo, bool) {
	return nil, false
}
...
func (m *mockCoordinator) NodeBecameUnavailable(node model.ServerInfo) {
	panic("not implemented")
}
