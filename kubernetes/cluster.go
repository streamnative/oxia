...
func (c *clusterClientImpl) applyCoordinator(cluster v1alpha1.OxiaCluster) error {
	...
	err := ServiceAccounts(c.kubernetes).Upsert(cluster.Namespace, serviceAccount(Coordinator, cluster))
	errs = multierr.Append(errs, err)

	err = Roles(c.kubernetes).Upsert(cluster.Namespace, role(cluster))
	errs = multierr.Append(errs, err)

	err = RoleBindings(c.kubernetes).Upsert(cluster.Namespace, roleBinding(cluster))
	errs = multierr.Append(errs, err)

	err = ConfigMaps(c.kubernetes).Upsert(cluster.Namespace, configMap(cluster))
	errs = multierr.Append(errs, err)

	err = Deployments(c.kubernetes).Upsert(cluster.Namespace, coordinatorDeployment(cluster))
	errs = multierr.Append(errs, err)

	err = Services(c.kubernetes).Upsert(cluster.Namespace, service(Coordinator, cluster, CoordinatorPorts))
	errs = multierr.Append(errs, err)

	if cluster.Spec.MonitoringEnabled {
		err = ServiceMonitors(c.monitoring).Upsert(cluster.Namespace, serviceMonitor(Coordinator, cluster))
		errs = multierr.Append(errs, err)
	}
...
func (c *clusterClientImpl) applyServers(cluster v1alpha1.OxiaCluster) error {
	...
	err := ServiceAccounts(c.kubernetes).Upsert(cluster.Namespace, serviceAccount(Server, cluster))
	errs = multierr.Append(errs, err)

	err = StatefulSets(c.kubernetes).Upsert(cluster.Namespace, serverStatefulSet(cluster))
	errs = multierr.Append(errs, err)

	err = Services(c.kubernetes).Upsert(cluster.Namespace, service(Server, cluster, ServerPorts))
	errs = multierr.Append(errs, err)

	if cluster.Spec.MonitoringEnabled {
		err = ServiceMonitors(c.monitoring).Upsert(cluster.Namespace, serviceMonitor(Server, cluster))
		errs = multierr.Append(errs, err)
	}
